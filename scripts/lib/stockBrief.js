...
async function buildAiContent(env, volumeList, changeList, dateLabel) {
...
export async function runStockBrief(env) {
  ...
  const aiContent = await buildAiContent(env, volumeList, changeList, dateLabel);
  ...
  return { success: true, title, usedAi: !!aiContent };
}
